...
{	 
	...
	for (double tem_PiPt = PiPt_MinOpt; tem_PiPt < PiPt_MaxOpt; tem_PiPt += (PiPt_MaxOpt - PiPt_MinOpt) / stepnum)
	{
		PiPt.setRange(tem_PiPt, PiPt_Max);
		PiPt2.setRange(tem_PiPt, PiPt_Max);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/PiPt","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/PiPt", "PiPt_merit_min", x_Data, y_Data);
		drawVectors("result/run3main_result/PiPt_mar", "PiPt_merit_min_mar", x_Data, y_Data, eventnum);
	}
	PiPt.setRange(PiPt_Min, PiPt_Max);
	PiPt2.setRange(PiPt_Min, PiPt_Max);// Reset range for next fit
}
