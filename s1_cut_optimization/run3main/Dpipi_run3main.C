...
{	 
	...
	for (double tem_Dpipi = Dpipi_MinOpt; tem_Dpipi < Dpipi_MaxOpt; tem_Dpipi += (Dpipi_MaxOpt - Dpipi_MinOpt) / stepnum)
	{
		Dpipi.setRange(tem_Dpipi, Dpipi_Max);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/Dpipi","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/Dpipi", "Dpipi_merit_min", x_Data, y_Data);
		drawVectors("result/run3main_result/Dpipi_mar", "Dpipi_merit_min_mar", x_Data, y_Data, eventnum);
	}
	Dpipi.setRange(Dpipi_Min, Dpipi_Max);// Reset range for next fit
}
