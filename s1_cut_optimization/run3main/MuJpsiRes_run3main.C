...
{	 
	...
	for (double tem_MuJpsiRes = MuJpsiRes_MinOpt; tem_MuJpsiRes < MuJpsiRes_MaxOpt; tem_MuJpsiRes += (MuJpsiRes_MaxOpt - MuJpsiRes_MinOpt) / stepnum)
	{
		MuJpsiRes.setRange(tem_MuJpsiRes, MuJpsiRes_Max);// Set cut as tem, which increase gradually
	...
	if(data->sumEntries() == 0)
		...
		{
			x_Data.push_back(tem_MuJpsiRes);
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/MuJpsiRes","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/MuJpsiRes", "MuJpsiRes_merit_min", x_Data, y_Data);
		drawVectors("result/run3main_result/MuJpsiRes_mar", "MuJpsiRes_merit_min_mar", x_Data, y_Data, eventnum);
	}
	MuJpsiRes.setRange(MuJpsiRes_Min, MuJpsiRes_Max);// Reset range for next fit
}
