...
void PiEta_run3main()
{
	...
	for (double tem_PiEta = PiEta_MaxOpt; tem_PiEta > PiEta_MinOpt; tem_PiEta -= (PiEta_MaxOpt - PiEta_MinOpt) / stepnum)
	{
		PiEta.setRange(PiEta_Min, tem_PiEta);// Set cut as tem, which increase gradually
		PiEta2.setRange(PiEta_Min, tem_PiEta);
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/PiEta","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/PiEta", "PiEta_merit_max", x_Data, y_Data);
		drawVectors("result/run3main_result/PiEta_mar", "PiEta_merit_max_mar", x_Data, y_Data, eventnum);
	}
	PiEta.setRange(PiEta_Min, PiEta_Max);
	PiEta2.setRange(PiEta_Min, PiEta_Max);// Reset range for next fit
}
