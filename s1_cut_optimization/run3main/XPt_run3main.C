...
{	 
	...
	for (double tem_XPt = XPt_MinOpt; tem_XPt < XPt_MaxOpt; tem_XPt += (XPt_MaxOpt - XPt_MinOpt) / stepnum)
	{
		XPt.setRange(tem_XPt, XPt_Max);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/XPt","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/XPt", "XPt_merit_min", x_Data, y_Data);
		drawVectors("result/run3main_result/XPt_mar", "XPt_merit_min_mar", x_Data, y_Data, eventnum);
	}
	XPt.setRange(XPt_Min, XPt_Max);// Reset range for next fit
}
