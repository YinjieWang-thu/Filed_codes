...
{	 
	...
	for (double tem_jpsi1Pt = jpsi1Pt_MinOpt; tem_jpsi1Pt < jpsi1Pt_MaxOpt; tem_jpsi1Pt += (jpsi1Pt_MaxOpt - jpsi1Pt_MinOpt) / stepnum)
	{
		jpsi1Pt.setRange(tem_jpsi1Pt, jpsi1Pt_Max);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/jpsi1Pt","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/jpsi1Pt", "jpsi1Pt_merit_min", x_Data, y_Data);
		drawVectors("result/run3main_result/jpsi1Pt_mar", "jpsi1Pt_merit_min_mar", x_Data, y_Data, eventnum);
	}
	jpsi1Pt.setRange(jpsi1Pt_Min, jpsi1Pt_Max);// Reset range for next fit
}
