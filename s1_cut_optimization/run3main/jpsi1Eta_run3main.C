...
void jpsi1Eta_run3main()
{
	...
	for (double tem_jpsi1Eta = jpsi1Eta_MaxOpt; tem_jpsi1Eta > jpsi1Eta_MinOpt; tem_jpsi1Eta -= (jpsi1Eta_MaxOpt - jpsi1Eta_MinOpt) / stepnum)
	{
		jpsi1Eta.setRange(jpsi1Eta_Min, tem_jpsi1Eta);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/jpsi1Eta","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/jpsi1Eta", "jpsi1Eta_merit_max", x_Data, y_Data);
		drawVectors("result/run3main_result/jpsi1Eta_mar", "jpsi1Eta_merit_max_mar", x_Data, y_Data, eventnum);
	}
	jpsi1Eta.setRange(jpsi1Eta_Min, jpsi1Eta_Max);// Reset range for next fit
}
