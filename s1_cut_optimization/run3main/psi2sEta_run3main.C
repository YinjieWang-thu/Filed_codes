...
void psi2sEta_run3main()
{
	...
	for (double tem_psi2sEta = psi2sEta_MaxOpt; tem_psi2sEta > psi2sEta_MinOpt; tem_psi2sEta -= (psi2sEta_MaxOpt - psi2sEta_MinOpt) / stepnum)
	{
		psi2sEta.setRange(psi2sEta_Min, tem_psi2sEta);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/psi2sEta","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/psi2sEta", "psi2sEta_merit_max", x_Data, y_Data);
		drawVectors("result/run3main_result/psi2sEta_mar", "psi2sEta_merit_max_mar", x_Data, y_Data, eventnum);
	}
	psi2sEta.setRange(psi2sEta_Min, psi2sEta_Max);// Reset range for next fit
}
