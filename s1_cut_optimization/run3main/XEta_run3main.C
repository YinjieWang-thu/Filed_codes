...
void XEta_run3main()
{
	...
	for (double tem_XEta = XEta_MaxOpt; tem_XEta > XEta_MinOpt; tem_XEta -= (XEta_MaxOpt - XEta_MinOpt) / stepnum)
	{
		XEta.setRange(XEta_Min, tem_XEta);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/XEta","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/XEta", "XEta_merit_max", x_Data, y_Data);
		drawVectors("result/run3main_result/XEta_mar", "XEta_merit_max_mar", x_Data, y_Data, eventnum);
	}
	XEta.setRange(XEta_Min, XEta_Max);// Reset range for next fit
}
