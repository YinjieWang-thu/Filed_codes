...
{	 
	...
	for (double tem_psi2sPt = psi2sPt_MinOpt; tem_psi2sPt < psi2sPt_MaxOpt; tem_psi2sPt += (psi2sPt_MaxOpt - psi2sPt_MinOpt) / stepnum)
	{
		psi2sPt.setRange(tem_psi2sPt, psi2sPt_Max);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/psi2sPt","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/psi2sPt", "psi2sPt_merit_min", x_Data, y_Data);
		drawVectors("result/run3main_result/psi2sPt_mar", "psi2sPt_merit_min_mar", x_Data, y_Data, eventnum);
	}
	psi2sPt.setRange(psi2sPt_Min, psi2sPt_Max);// Reset range for next fit
}
