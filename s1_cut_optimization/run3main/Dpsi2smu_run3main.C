...
void Dpsi2smu_run3main()
{
	...
	for (double tem_Dpsi2smu = Dpsi2smu_MaxOpt; tem_Dpsi2smu > Dpsi2smu_MinOpt; tem_Dpsi2smu -= (Dpsi2smu_MaxOpt - Dpsi2smu_MinOpt) / stepnum)
	{
		Dpsi2smu.setRange(Dpsi2smu_Min, tem_Dpsi2smu);
		Dpsi2smu2.setRange(Dpsi2smu_Min, tem_Dpsi2smu);
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/Dpsi2smu","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/Dpsi2smu", "Dpsi2smu_merit_max", x_Data, y_Data);
		drawVectors("result/run3main_result/Dpsi2smu_mar", "Dpsi2smu_merit_max_mar", x_Data, y_Data, eventnum);
	}
	Dpsi2smu.setRange(Dpsi2smu_Min, Dpsi2smu_Max);
	Dpsi2smu2.setRange(Dpsi2smu_Min, Dpsi2smu_Max);// Reset range for next fit
}
