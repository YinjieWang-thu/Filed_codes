...
void jpsi2Eta_run3main()
{
	...
	for (double tem_jpsi2Eta = jpsi2Eta_MaxOpt; tem_jpsi2Eta > jpsi2Eta_MinOpt; tem_jpsi2Eta -= (jpsi2Eta_MaxOpt - jpsi2Eta_MinOpt) / stepnum)
	{
		jpsi2Eta.setRange(jpsi2Eta_Min, tem_jpsi2Eta);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/jpsi2Eta","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/jpsi2Eta", "jpsi2Eta_merit_max", x_Data, y_Data);
		drawVectors("result/run3main_result/jpsi2Eta_mar", "jpsi2Eta_merit_max_mar", x_Data, y_Data, eventnum);
	}
	jpsi2Eta.setRange(jpsi2Eta_Min, jpsi2Eta_Max);// Reset range for next fit
}
