...
void Dpsi2spi_run3main()
{
	...
	for (double tem_Dpsi2spi = Dpsi2spi_MaxOpt; tem_Dpsi2spi > Dpsi2spi_MinOpt; tem_Dpsi2spi -= (Dpsi2spi_MaxOpt - Dpsi2spi_MinOpt) / stepnum)
	{
		Dpsi2spi.setRange(Dpsi2spi_Min, tem_Dpsi2spi);
		Dpsi2spi2.setRange(Dpsi2spi_Min, tem_Dpsi2spi);// Set cut as tem, which increase gradually
	...
	//	print("numB", numB);// Background number
		...
		print_vectors("result/run3main_result/Dpsi2spi","Psi2s_SfromData", x_Data, y_Data, eventnum);
		drawVectors("result/run3main_result/Dpsi2spi", "Dpsi2spi_merit_max", x_Data, y_Data);
		drawVectors("result/run3main_result/Dpsi2spi_mar", "Dpsi2spi_merit_max_mar", x_Data, y_Data, eventnum);
	}
	Dpsi2spi.setRange(Dpsi2spi_Min, Dpsi2spi_Max);
	Dpsi2spi2.setRange(Dpsi2spi_Min, Dpsi2spi_Max);// Reset range for next fit
}
