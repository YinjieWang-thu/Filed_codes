...
void plot_var_baseline(bool writeBDTFile=true) {
  ...
  newtree_sig->Branch("pt_mu1",&pt_mu1,"pt_mu1/F");
  newtree_sig->Branch("pt_mu2",&pt_mu2,"pt_mu2/F");
  newtree_sig->Branch("pt_K_X",&pt_K_X,"pt_K_X/F");
  newtree_sig->Branch("pt_pi_X",&pt_pi_X,"pt_pi_X/F");
  newtree_sig->Branch("dR_mumu",&dR_mumu,"dR_mumu/F");
  newtree_sig->Branch("dR_Kpi",&dR_Kpi,"dR_Kpi/F");
  newtree_sig->Branch("dR_JpsiKst",&dR_JpsiKst,"dR_JpsiKst/F");
  newtree_sig->Branch("pt_LRT_trk1",&pt_LRT_trk1,"pt_LRT_trk1/F");
  newtree_sig->Branch("pt_LRT_trk2",&pt_LRT_trk2,"pt_LRT_trk2/F");
  newtree_sig->Branch("jx_rf_pt",&jx_rf_pt,"jx_rf_pt/F");
  newtree_sig->Branch("ld_rf_pt",&ld_rf_pt,"ld_rf_pt/F");
  newtree_sig->Branch("dR_pipi",&dR_pipi,"dR_pipi/F");
  newtree_sig->Branch("dR_B0Ks",&dR_B0Ks,"dR_B0Ks/F");
  newtree_sig->Branch("tq_rf_pt",&tq_rf_pt,"tq_rf_pt/F");
  newtree_sig->Branch("tq_rf_m",&tq_rf_m,"tq_rf_m/F");
  newtree_sig->Branch("jpsi_m",&jpsi_m,"jpsi_m/F");
  newtree_sig->Branch("x_rf_m",&x_rf_m,"x_rf_m/F");
  newtree_sig->Branch("jx_m_calc",&jx_m_calc,"jx_m_calc/F");	  
  newtree_sig->Branch("ld_m",&ld_m,"ld_m/F");
  ...
  newtree_bkg->Branch("pt_mu1",&pt_mu1,"pt_mu1/F");
  newtree_bkg->Branch("pt_mu2",&pt_mu2,"pt_mu2/F");
  newtree_bkg->Branch("pt_K_X",&pt_K_X,"pt_K_X/F");
  newtree_bkg->Branch("pt_pi_X",&pt_pi_X,"pt_pi_X/F");
  newtree_bkg->Branch("dR_mumu",&dR_mumu,"dR_mumu/F");
  newtree_bkg->Branch("dR_Kpi",&dR_Kpi,"dR_Kpi/F");
  newtree_bkg->Branch("dR_JpsiKst",&dR_JpsiKst,"dR_JpsiKst/F");
  newtree_bkg->Branch("pt_LRT_trk1",&pt_LRT_trk1,"pt_LRT_trk1/F");
  newtree_bkg->Branch("pt_LRT_trk2",&pt_LRT_trk2,"pt_LRT_trk2/F");
  newtree_bkg->Branch("jx_rf_pt",&jx_rf_pt,"jx_rf_pt/F");
  newtree_bkg->Branch("ld_rf_pt",&ld_rf_pt,"ld_rf_pt/F");
  newtree_bkg->Branch("dR_pipi",&dR_pipi,"dR_pipi/F");
  newtree_bkg->Branch("dR_B0Ks",&dR_B0Ks,"dR_B0Ks/F");
  newtree_bkg->Branch("tq_rf_pt",&tq_rf_pt,"tq_rf_pt/F");
  newtree_bkg->Branch("tq_rf_m",&tq_rf_m,"tq_rf_m/F");
  newtree_bkg->Branch("jpsi_m",&jpsi_m,"jpsi_m/F");
  newtree_bkg->Branch("x_rf_m",&x_rf_m,"x_rf_m/F");
  newtree_bkg->Branch("jx_m_calc",&jx_m_calc,"jx_m_calc/F");	  
  newtree_bkg->Branch("ld_m",&ld_m,"ld_m/F");
  ...
  for(Int_t ich=0; ich<3; ich++) {
	...
	idx = 3;
	  ...
	  //(tq_rf_m/GeV>5.80 && tq_rf_m/GeV<5.832)
	  //|| (tq_rf_m/GeV<5.80)
	  ) {
	Float_t wt = 1;
  ...
  for(Int_t i=0; i<NN; i++) {    
    ...
    if(subtractBkg) h[i][3]->Add(hmix[i],-1);
    else htot[i]->Add(hmix[i]);
  ...
  for(Int_t i=0; i<NN; i++) {
    ...
    hratio[i]->Draw("EX0");
     ...
     sprintf(str,"./plot/baseline_cut/c%d.png",i+1);
     c[i]->SaveAs(str);
  }

  if(writeBDTFile) {
    TFile newfile("inputfile2.root", "recreate");
    newtree_sig->Write();
    newtree_bkg->Write();
  }
}
